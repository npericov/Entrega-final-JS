...
function agregarCarrito(id){
    const poleras = Catalogo.find((polera) => polera.id === id);

    if(poleras && !productosAgregados.some((item) => item.id === id)){
        ...
        productosAgregados.push(poleras);
...
//----------- BORRAR DE LA LISTA
const BooklistElement = document.querySelector('.productslist');
...
function deleteBook(id){
    ...
    if(index !== -1){
        ...
        const BooklistElement = document.querySelector('.productslist'); //Elimina elemento del carrito en el DOM
